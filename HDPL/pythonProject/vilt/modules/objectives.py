...
def compute_modmis_bin(pl_module, batch):  # 针对二分类输出  分类BCE
    ...
    if pl_module.hparams.config["used_loss"] == "bcewl":
        ret[f"{loss_name}_loss"] = F.binary_cross_entropy_with_logits(
            ...
            ret[f"{loss_name}_labels"].float().unsqueeze(1),
...
def compute_modmis_reg(pl_module, batch):  # 回归头输出  回归MSE
    ...
    ret = {f"{loss_name}_labels": torch.stack(batch['label']['field'].int),}
...
#         # 对于 nn.Linear, nn.Embedding, 和 nn.Conv1d 使用正态分布初始化权重
#         module.weight.data.normal_(mean=0.0, std=0.02)
#         if isinstance(module, nn.Conv1d):
#             # 对于卷积层，初始化偏置为一个较小的值，例如 0.01
#             if module.bias is not None:
#                 module.bias.data.fill_(0.01)
#
#     elif isinstance(module, nn.LayerNorm):
#         # 对于 nn.LayerNorm，初始化偏置为 0，权重为 1.0
#         module.bias.data.zero_()
#         module.weight.data.fill_(1.0)
#
#     # 对于 nn.BatchNorm1d，初始化权重为 1.0，偏置为 0
